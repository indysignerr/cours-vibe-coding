...
export function SiteFooter() {
  return (
    <footer className="mx-auto max-w-stage px-5 pb-28 pt-14 sm:pb-14 md:px-10 md:pt-24">
      <div className="card-3d flex flex-wrap items-end justify-between gap-8 p-6 md:p-8">
        ...
        <nav aria-label="Secondary" className="flex flex-wrap items-center gap-x-1 gap-y-1 text-sm">
          ...
          {LINKS.map(([href, label]) => (
            <Link key={href} className="tap flex items-center rounded-full px-3 font-bold text-muted hover:bg-sunken hover:text-ink" href={href}>
              {label}
            </Link>
          ))}
        </nav>
